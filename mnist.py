...
from circleloss import LabelCircleLossModel
...
def main(resume: bool = True) -> None:
    ...
    criterion = LabelCircleLossModel(num_classes=10, m=0.25, gamma=256,feature_dim=32)
    ...
    if resume and os.path.exists("resume.state"):
        ...
        for epoch in range(20):
            for img, label in tqdm(train_loader):
                model.zero_grad()
                pred = model(img.cuda())
                loss,sp,sn = criterion(pred, label.cuda())
                loss.backward()
                optimizer.step()
